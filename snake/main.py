...
def snake():
    ...
    import pygame_menu
    # import sys
    # from main import mmenu

    # sys.path.insert(0,"/my folder/Сашин проект Орленок/main.py")
    ...
    def message(msg, color):
        ...
        screen.blit(msg, (W//2 - msg.get_width() //
                    2, H//2 - msg.get_height()//2))
    ...
    def draw_sound(has_sound):
        sound = pygame.image.load('./snake/assets/sound.svg')
        nosound = pygame.image.load('./snake/assets/nosound.svg')
        if has_sound:
    ...
    def draw_snake(snake_list, color):
        for el in snake_list:
            pygame.draw.rect(
                screen, color, (el[0], el[1], snake_block, snake_block))

    def generate_food():
        x = round(random.randrange(0, W - snake_block) /
                  snake_block) * snake_block
        y = round(random.randrange(0, H - snake_block) /
                  snake_block) * snake_block
    ...
    color = None
    pygame.init()
    pygame.mixer.music.load(f'./snake/sounds/{get_all_sounds()[0]}')
    ...
    def set_music(name, value):
        pygame.mixer.music.load(f'./snake/sounds/{value}')
    ...
    menu = pygame_menu.Menu('Змейка', W, H,
                            theme=pygame_menu.themes.THEME_BLUE)

    menu.add.selector('Музыка: ', list(
        map(lambda x: (x.split('.')[0], x), get_all_sounds())), onchange=set_music)
    menu.add.color_input('Цвет змейки', color_type='hex',
                         default='ff0000', onchange=choose_color)
    ...
    while running:
        ...
        for rock in ROCKS_LIST:
            pygame.draw.rect(
                screen, BLACK, [rock[0], rock[1], snake_block, snake_block])
