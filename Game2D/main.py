def platform():
    ...
    image_path = './Game2D/data/data/ghost hunting.myapp/files/app/'
    ...
    pygame.display.set_caption("Game2D")         # - Название проекта
    icon = pygame.image.load( './Game2D/images/game_icon.png').convert_alpha()   # - Команда для создания иконки игры указывая картинку на картинку из определенной папки
    pygame.display.set_icon(icon)                      # - указываем дисплею нашу иконку

    bg = pygame.image.load('./Game2D/images/bg.png').convert_alpha()  # - установление фона
    walk_left = [ # - анимация влево через картинки
        pygame.image.load('./Game2D/images/player_left/left1.png').convert_alpha(),
        pygame.image.load('./Game2D/images/player_left/left2.png').convert_alpha(),
        pygame.image.load('./Game2D/images/player_left/left3.png').convert_alpha(),
        pygame.image.load('./Game2D/images/player_left/left4.png').convert_alpha(),
    ]

    walk_right = [ # - анимация вправо  через картинки                                                
        pygame.image.load('./Game2D/images/player_right/right1.png').convert_alpha(),
        pygame.image.load('./Game2D/images/player_right/right2.png').convert_alpha(),
        pygame.image.load('./Game2D/images/player_right/right3.png').convert_alpha(),
        pygame.image.load('./Game2D/images/player_right/right4.png').convert_alpha(),
    ]

    ghost = pygame.image.load('./Game2D/images/ghost.png').convert_alpha() # - подключение картинки приведения к коду .convert_alpha() - для того чтобы конвертировать картинки
    ...
    bg_sound = pygame.mixer.Sound('./Game2D/sounds/bg.mp3')     # - музыка на фоне  то есть при включении игры музыка сразу же включается
    bg_sound.play()

    ghost_timer = pygame.USEREVENT + 1
    pygame.time.set_timer(ghost_timer, 2500)           # - количество секунд между поевлением след. приведения

    label = pygame.font.Font('./Game2D/fonts/Roboto-Black.ttf', 40)
    ...
    bullets_left = 5
    bullet = pygame.image.load('./Game2D/images/bullet.png').convert_alpha()
    bullets = [ ]
